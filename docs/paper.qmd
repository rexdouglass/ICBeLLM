...
table1 <- table1 %>% mutate_all(str_replace_all,"\r\n", " ") %>% mutate_all(replace_na, '') 
kable_output <- kable(table1, caption = "Table 1: Comparison of ICBe to ICBeLLM Workflows", "html", align = c("l", "l", "l", "l", "l")) %>%
  kable_styling(font_size = 10, full_width = FALSE) %>%
  column_spec(1, width = "15em") %>%  # Adjust the width as necessary
...
#kable_output %>%  save_kable("/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/paper/paper2_temp_table.png")

```

```{r, eval=T,output=T}

Sys.setenv("OPENSSL_CONF"="/dev/null") #https://stackoverflow.com/questions/68787119/issue-with-webshot-js-in-r
#, results='asis'
#print(kable_output)
kable_output %>%   kableExtra::as_image(width = 8)
...
icb_events_long <- readRDS(file="/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_in/ICBe_V1.1_long_clean.Rds")   #we need the clean version specifically
#Borrow the qcode mappings from the icbe long version
value_normalized_to_value_qcode <- icb_events_long %>% dplyr::select(value_normalized, value_qcode) %>% distinct() %>% filter(!is.na(value_qcode)) %>%
  mutate(value_qcode_clean = paste0("![](/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_in/flags_small/",value_qcode,".png)")) %>%
...
in_files %>% lapply(read_csv) %>% lapply(function(df) mutate_all(df, as.character)) %>% bind_rows()  %>%
        arrange(crisno,sentence_number,event_number, event_number_subevent) %>% write_csv("/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_out/icbe_all_codings.csv", na = "")
ICBeLLM_all_codings <- read_csv("/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_out/icbe_all_codings.csv")
...
#View(table196)
table196 %>% filter(!is.na(thought_leaf) & event_number_subevent==1) %>% dplyr::select(sentence_clean, event_clean, think_actor_a, thought_leaf, quality_control_thought)
thoughts <- table196 %>% 
...
table196 %>% filter(!is.na(speech_leaf) & event_number_subevent==1) %>% dplyr::select(sentence_clean, event_clean,  say_actor_a, speech_leaf, say_actor_b, quality_control_say)
speech <- table196 %>% filter(!is.na(speech_leaf)  & event_number_subevent==1 ) %>% #we're going to mechanically require it to be subevent 1
...
do <- table196 %>% filter( !is.na(do_leaf)) %>%
      mutate(do_leaf='') %>%
      ...
      mutate(interact_fatalities=ifelse(interact_fatalities!="none","![](/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_in/flags_small/Q1056901.png)",interact_fatalities)) %>%
      mutate(interact_forces=ifelse(interact_forces!="none","![](/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_in/flags_small/Q176799.png)",interact_fatalities)) %>%
...
combined <- bind_rows(
      table196 %>%  dplyr::select(crisno, sentence_number,event_number, event_number_subevent,  sentence_clean, sentence, event_clean) %>% distinct(),
...
tp_196 <- temp %>% #screen duplicate events here
  flextable() %>%
  set_caption("Table 2: Cuban Missile Crisis") %>%
  ...
  colformat_md(j=3) %>%
  merge_v( j = 1) %>%
  merge_v( j = 2) %>% #merge duplicates if multiple events per event
  #merge_v( j = 2) %>% #breaks things
  set_header_labels(col_keys = c("sentence_clean", "event_clean","icbellm_icons"), values = c("Original Sentence", "LLM Event Split","ICBeLLM Codings") ) %>%
  padding(padding.top = 0, padding.bottom = 0) %>%
  hline(j=1,i=c(1,which(temp$sentence_clean!=lag(temp$sentence_clean))), part = "body") %>% #, border = officer::fp_border(color = "gray")
  hline(j=2,i=c(2,which(temp$sentence_clean!=lag(temp$sentence_clean)))-1, part = "body")  %>% #, border = officer::fp_border(color = "gray")
  hline(j=3,i=c(2,which(temp$sentence_clean!=lag(temp$sentence_clean)))-1, part = "body") #  , border = officer::fp_border(color = "gray")
...
events_agreed_long_filtered_experts_atleast2 <-  events_agreed_long %>%
                                                 filter(sentence_span_text %>% str_detect("\\.$"))  %>% filter(expert==1) %>% #Removes 626 degenerate sentence 
...
ICBeLLM %>%
  ...
  #knitr::kable() %>%
  knitr::kable( caption = "Table 3: Quality Control Acceptance Rate", "html", align = c("l", "l")) %>%
  kableExtra::kable_styling(font_size = 10, full_width = FALSE) #%>%
  #kableExtra::footnote(general = "Your footnote here.") 
...
recall_df_raw_actors %>% write_csv("/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_out/actors_to_recode.csv")
...
recall_df_raw_thought_leaf %>% write_csv("/media/skynet3/8tb_a/rwd_github_private/ICBeLLM/data_out/thought_leaf.csv")
...
# Calculate and format recall
  ...
  kable( caption = "Table 4: Recall by Variable", "html", align = c("l", "l", "l", "l", "l")) %>%
  kable_styling(font_size = 10, full_width = FALSE) %>%
# Add footnote
kableExtra::footnote("") #
